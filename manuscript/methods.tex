The estimator is the locked SPGD stack \citep{Fu2026spgd} plus two operators: a KEEP/ABSTAIN reportability gate and a patient-identity donor split (Fig.~\ref{fig:protocol}A--F). Related mixed-spot readers recover composition, including RCTD, cell2location, DestVI, and CARD~\citep{Cable2022,Kleshchevnikov2022,Lopez2022,Ma2022}. This protocol adds an explicit rule for when the malignant coordinate is reportable.
...
\end{equation}
Marker genes receive large \(w_g\) under \eqref{eq:specificity}; genes shared across neighboring types receive small \(w_g\). The weighted signature and the weighted spot matrix are \(P\odot w\) and \(Y\odot w\).

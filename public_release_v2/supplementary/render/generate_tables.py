...
ITEM_LABEL = {
    ...
    "xenium_ref": "Breast Xenium reference",
    "xenium_flex_ref": "Xenium FLEX orthogonal reference",
    "cosmx_ref": "CosMx BCC reference",
    "donor_lock_ref": "CosMx donor-lock (recorded)",
    "donor_transfer_row.csv": "Donor-transfer row",
...
# donor_lock carries the donor lock rather than an evaluated substrate, so its
# CSV role would otherwise read as a benchmark this paper scores against.
ROLE_OVERRIDE = {"donor_lock_ref": "recorded"}
# Substrate codenames as they appear inside file names and table cells. Longest
# first: a bare "xenium" prefixes the numbered ones.
SUBSTRATE_LABEL = {"cosmx": "CosMx", "xenium_flex": "Xenium FLEX", "xenium": "Xenium", "openst": "openST"}
...
def main() -> None:
    ...
    throughput = []
    for key in ("openst", "cosmx", "xenium"):

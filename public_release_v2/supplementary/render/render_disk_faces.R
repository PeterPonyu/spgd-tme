...
source(file.path(.here, "spatial_geom.R"))
...
orth <- read.csv(file.path(plotdir, "F2_xenium_flex_cosine.csv"), stringsAsFactors = FALSE)
...
if (!identical(Sys.getenv("TME_RENDER_ONLY"), "F11")) {
  source(file.path(.here, "render_metric_faces.R"))
}
source(file.path(.here, "render_spatial_faces.R"))
